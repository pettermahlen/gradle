...
link:{javadocPath}/org/gradle/api/provider/ValueSource.html[`ValueSource`] can be used if necessary, at the cost of making the process output an input to the build configuration.
...
To fix this deprecation requires one of two things:
...
* Or change the return type of the method to `boolean`.
...
For example, given this old code:
...
This should not impact catalogs that use the <<version_catalogs.adoc#sub::toml-dependencies-format,recommended syntax>> or were generated by Gradle for publication.
